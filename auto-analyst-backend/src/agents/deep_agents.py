import dspy
import asyncio
from agents import planner_data_viz_agent, planner_preprocessing_agent, planner_sk_learn_agent, planner_statistical_analytics_agent
from agents import data_viz_agent,preprocessing_agent,statistical_analytics_agent,sk_learn_agent
from agents import PLANNER_AGENTS_WITH_DESCRIPTION, AGENTS_WITH_DESCRIPTION
import ast
import json
import markdown
...
class deep_analysis_module(dspy.Module):
    ...
    async def execute_deep_analysis(self,goal, dataset_info):
        ...
        #     except Exception as e:
        #         attempt += 1
        #         # Extract error details in a structured way
        #         error_info = {
        ...
        #         if hasattr(e, 'lineno'):
        #             error_info['error_location'] = e.lineno
        #             code_lines = code.split('\n')
        #             start_line = max(0, e.lineno - 3)
        #             end_line = min(len(code_lines), e.lineno + 3)
        ...
        #             print(f"Error occurred around line {error_info['error_location']}")
        #             print("\nCode context around error:")
        #             print("=" * 80)
        #             print(error_info['code_context'])
        #             print("=" * 80)
                
        #         if attempt == max_attempts:
        #             raise Exception(f"Failed to fix code after {max_attempts} attempts")
                
        #         # Try to fix the code
        #         fixed = self.deep_code_fixer(
        #             code=code,
        #             error=str(json.dumps(error_info, indent=2))
        #         )
                
        #         if not hasattr(fixed, 'fixed_code'):
        #             raise ValueError("Expected fixed_code attribute in response")
...
query = "What is happening to churn ?"
